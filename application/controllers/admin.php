...
public function buat_surat() {
		...
		/* pagination */	
		$total_row		= $this->db->query("SELECT * FROM t_buat_surat")->num_rows();
		$per_page		= 10;
		...
		$a['pagi']	= _page($total_row, $per_page, 4, base_url()."admin/buat_surat/p");
		...
		if ($mau_ke == "del") {
			$this->db->query("DELETE FROM t_buat_surat WHERE id = '$idu'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been deleted </div>");
			redirect('admin/buat_surat');
		} else if ($mau_ke == "cari") {
			$a['data']		= $this->db->query("SELECT * FROM t_buat_surat WHERE isi_ringkas LIKE '%$cari%' ORDER BY id DESC")->result();
			$a['page']		= "l_buat_surat";
		} else if ($mau_ke == "add") {
			$a['page']		= "f_buat_surat";
		} else if ($mau_ke == "edt") {
			$a['datpil']	= $this->db->query("SELECT * FROM t_buat_surat WHERE id = '$idu'")->row();	
			$a['page']		= "f_buat_surat";
		} else if ($mau_ke == "act_add") {	
			$this->db->query("INSERT INTO t_buat_surat VALUES (NULL, '$kode', '$no_agenda', '$uraian', '$tujuan', '$no_surat', '$tgl_surat', NOW(), '$ket', '".$this->session->userdata('admin_id')."')");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been added</div>");
			redirect('admin/buat_surat/');
		} else if ($mau_ke == "act_edt") {
			$this->db->query("UPDATE t_buat_surat SET kode = '$kode', no_agenda = '$no_agenda', isi_ringkas = '$uraian', tujuan= '$tujuan', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket' WHERE id = '$idp'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been updated</div>");			
			redirect('admin/buat_surat');
		} else {
			$a['data']		= $this->db->query("SELECT * FROM t_buat_surat LIMIT $awal, $akhir ")->result();
			$a['page']		= "l_buat_surat";
		}
...
public function preview_cetak() {
		$idu = $this->uri->segment(3);
		$a['datpil1']	= $this->db->query("SELECT * FROM t_buat_surat WHERE id = '$idu'")->row();	
		$this->load->view('admin/f_preview_cetak', $a);
	}
}